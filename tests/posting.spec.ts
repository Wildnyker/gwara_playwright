...
test.describe("Positive cases for adding a post as user", () => {
  test("Add a valid post with title only", async ({ page }) => {
    ...
    await expect(
      pm.onPostPage().postTitle,
      "Title has expected text"
    ).toHaveText("Test title 1");
  ...
  test("Add a valid post with title + body", async ({ page }) => {
    ...
    await expect(
      pm.onPostPage().postTitle,
      "Title has expected text"
    ).toHaveText("Test title 2");
    await expect(pm.onPostPage().postBody, "Body has expected text").toHaveText(
      "Test Body"
    );
  });
  ...
  test("Add a valid post with title + body + image", async ({ page }) => {
    ...
    await pm
      ...
      .addPost(
        "Test title 3",
        "https://clipart-library.com/data_images/320464.png",
        "Test Body 2"
      );
    ...
    await expect(
      pm.onPostPage().postTitle,
      "Title has expected text"
    ).toHaveText("Test title 3");
    await expect(pm.onPostPage().postImage, "Image is visible").toBeVisible();
    await expect(
      pm.onPostPage().postImage,
      "Image has expected src"
    ).toHaveAttribute(
      "src",
      "https://clipart-library.com/data_images/320464.png"
    );
...
test.describe("Negative cases for adding a post as user", () => {
  test.use({ storageState: ".auth/user1.json" });
  test("Add a post with image + body without title(required)", async ({
    page,
  }) => {
  ...
  test("Add a post with a non-valid image link", async ({ page }) => {
    ...
    await expect(page).toHaveURL("/");
    await expect(pm.onFeedPage().addPostError).toHaveText(
...
test.describe("Post deletion", () => {
  test("delete own post", async ({ page }) => {
    ...
    await pm
      .onPostPage()
      .verifyCorrectPostIsOpened("Post title 1", TESTUSER_1_NAME);
    ...
    await expect(
      pm.onFeedPage().postFeedTitle("Post title 1"),
      "Check that post is no longer visible on the global Feed page"
    ).not.toBeVisible();
  });
  ...
  test("delete other user's post", async ({ page }) => {
    ...
    await pm
      .onPostPage()
      .verifyCorrectPostIsOpened("Post title 2", TESTUSER_2_NAME);

    //Assert: Check that no delete button is visible for the other user's posts
    await expect(
      pm.onPostPage().postDeleteButton,
      "No delete button is visible for the other user's posts"
    ).not.toBeVisible();
  });
});
...
test.describe("Feed Empty state", () => {
  test("Verify empty state is shown", async ({ page }) => {
    ...
    await expect(pm.onFeedPage().emptyPostsContainer).toHaveText(
      "Жодного допису. "
    );
