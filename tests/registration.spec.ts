...
test("valid registration", async ({ page }) => {
  ...
  await pm
    .onRegistrationPage()
    .register(validUserName, validUserPassword, validUserPassword);

  //Assert: Ensure we're redirected to the main page & registered user's name is visible
  await expect(page, "Redirected to main page after the login").toHaveURL("/");
  await page.locator("#menu-button").click();
  await expect(
    page.getByText(validUserName),
    "Name of the logged in user is visible"
  ).toBeVisible();
});
...
test("short password registration", async ({ page }) => {
  ...
  await pm
    .onRegistrationPage()
    .register(validUserName, INVALID_SHORT_TEST_PASS, INVALID_SHORT_TEST_PASS);

  // Assert: Correct error is show
  await expect(await pm.onRegistrationPage().finalValidationError).toHaveText(
...
test("too known password registration", async ({ page }) => {
  ...
  await pm
    .onRegistrationPage()
    .register(validUserName, knownPassword, knownPassword);
...
test("password from integers only registration", async ({ page }) => {
  ...
  await pm
    .onRegistrationPage()
    .register(validUserName, integersPassword, integersPassword);
...
test("passwords do not match registration", async ({ page }) => {
  ...
  await pm
    .onRegistrationPage()
    .register(validUserName, validPassword1, validPassword2);
...
test("existing username registration", async ({ page }) => {
  ...
  await pm
    .onRegistrationPage()
    .register(TESTUSER_1_NAME, validPassword, validPassword);
