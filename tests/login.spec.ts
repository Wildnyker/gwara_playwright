...
test("Valid login", async ({ page }) => {
  ...
  await pm
    .onLoginPage()
    .fillTheFieldsAndLogIn(TESTUSER_1_NAME, VALID_TEST_PASS);
  ...
  await expect(
    page.locator(".dropdown-item-text", { hasText: TESTUSER_1_NAME }),
...
test("invalid password login", async ({ page }) => {
  ...
  await pm
    .onLoginPage()
    .fillTheFieldsAndLogIn("Tester", INVALID_SHORT_TEST_PASS);
  ...
  await expect(pm.onLoginPage().errorMessage).toHaveText(
    "Йой, помилка в імені або паролі."
  );
});
