...
test.describe("Authenticated user comment tests", () => {
  ...
  test("empty comment is not added; existing comment stays first", async ({
    page,
  }) => {
    //Arrange
    const pm = new PageManager(page);
  ...
  test("add max-length comment", async ({ page }) => {
    ...
    //Assert
    await expect(pm.onPostPage().firstCommentBody).toContainText(
      MAX_LENGTH_COMMENT
    );
  ...
  test("edit own comment ", async ({ page }) => {
    ...
    await pm
      .onPostPage()
      .verifyCorrectPostIsOpened("Post title 4", TESTUSER_1_NAME);
...
test.describe("Anonymous user tests", () => {
  ...
  test("check comment posting is not possible for not logged user", async ({
    browser,
  }) => {
    //Arrange
