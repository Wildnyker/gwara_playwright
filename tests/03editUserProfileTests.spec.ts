...
test('should submit empty "About Me" form and show empty state', async({page})=>{
    ...
    await expect(pm.onEditAboutMePage().editAboutMePageHeader).toHaveText(`Редагувати сторінку користувача ${TESTUSER_2_NAME}`);
    await expect (pm.onEditAboutMePage().aboutMeTextbox).toHaveAttribute("placeholder","Про мене");
...
test('should save and display entered "About Me" text', async({page})=>{
    ...
    await expect(pm.onAboutMePage().aboutMeDescription).toHaveText("Про мене: Test description");
})
...
test('should update existing "About Me" text with new value', async({page})=>{
    ...
    await expect(pm.onEditAboutMePage().aboutMeTextbox).toHaveText('Test description');
    ...
    await expect(page.url).toBe(`/accuonts/u/${TESTUSER_2_NAME}`);
    await expect(pm.onAboutMePage().aboutMeDescription).toHaveText("Про мене: New description");
})
...
test('should correctly save and display max-length "About Me" text', async({page})=>{
    ...
    await expect(pm.onEditAboutMePage().aboutMeTextbox).toHaveText(LONG_ABOUT_ME_TEXT);
    ...
    await expect(page.url).toBe(`/accuonts/u/${TESTUSER_2_NAME}`)
    await expect(pm.onAboutMePage().aboutMeDescription).toHaveText(`Про мене: ${LONG_ABOUT_ME_TEXT}`);
})
...
test('should truncate and save "About Me" when text exceeds max length', async({page})=>{
    ...
    await expect(pm.onEditAboutMePage().aboutMeTextbox).toHaveText(TOO_LONG_ABOUT_ME_TEXT);
