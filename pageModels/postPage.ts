...
export class PostPage {
  ...
  constructor(page: Page) {
    ...
    this.postBody = this.page.locator(
      '//p[@class="entry-text"]/following-sibling::p[1]'
    );
    ...
    this.postCommentEditButton = this.page.locator(
    ...
    this.commentDeletionPopupTitle = this.page.getByText(
    ...
    this.confirmPostDeletionButton = this.page.getByRole("button", {
      name: "Так,прибрати",
    });
    this.cancelPostDeletionButton = this.page.getByRole("button", {
      name: "Ні, назад",
    });
    this.deletionConfirmationPopupTitle = this.page.locator(".entry h4", {
      hasText: "Ви впевнені, що хочете прибрати цей допис?",
    });
  ...
  async verifyCorrectPostIsOpened(title: string, author: string) {
    await expect(
      ...
      "Check that post page is opened"
    ).toBeVisible();
    await expect(
      this.postTitle,
      "Check that correct post title is shown"
    ).toHaveText(title);
    await expect(
      this.postAuthor,
      "Check that correct post author is shown"
    ).toHaveText(author);
  }
  ...
  async clickDeletePostButton() {
    ...
    await expect(
      this.deletionConfirmationPopupTitle,
      "Check that deletion popup is opened"
    ).toBeVisible();
  }
  async confirmPostDeletion() {
    ...
    await expect(
      this.postDeletedTag,
      "Check that post is marked as deleted"
    ).toBeVisible();
